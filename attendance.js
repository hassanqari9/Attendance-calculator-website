...
// public:
    ...
    // set_Attended()
    // {
    //     Classes_attended = this.a;
    // }

    // set_Total_classes_held()
    // {
    //     Total_classes_held = this.b;
    // }
    ...
    get_Attendance_percentage()
    {
        ...
        {
            document.querySelector(".text3").innerHTML="Current attendance: "+(a1.get_Attendance_ratio() * 100)+"%";
    ...
    Argument_verifier()
    {
        if (this.Criteria <= 0 || this.Classes_attended < 0 || this.Total_classes_held <= 0 || this.Classes_attended > this.Total_classes_held)
        {
    ...
    selector()
    {
        if (a1.get_Attendance_ratio() < this.Criteria)
            a1.shortage();
    ...
    {
        var trigger = 0, i = 0;
        var variable_attendance_ratio = 0, m, n;
        ...
        for (var i = 0; i < 36500; i++)
        {
            ...
            if (variable_attendance_ratio >= this.Criteria)
            {
                ...
                case 1: // Test Case(rare case): 75/7.4/10 (could not find whole numbers)
                    document.querySelector(".text2").innerHTML="You need to attend " + i + " more class to get attendance of " +(i + this.Classes_attended) / (i + this.Total_classes_held) * 100+ "%.";
                    trigger = 10;
                    break;

                default: // Test Case(normal case): 75/5/10
                    document.querySelector(".text2").innerHTML="You need to attend " + i + " more classes to get attendance above " + this.Criteria * 100 + "%, after which your attendace will be: " +(i + this.Classes_attended) / (i + this.Total_classes_held) * 100+ "%.";
                    trigger = 10;
                    break;
                }
                break;
    ...
    excess()
    {
        ...
        for (var i = 0; i < 36500; i++)
        {
            ...
            if (variable_attendance_ratio < this.Criteria)
            {
                ...
                {
                    document.querySelector(".text2").innerHTML="You may skip no classes. If you skip a class your attendance will be: " +(this.Classes_attended / (this.Total_classes_held + 1)) * 100+ "%."; // Test Case: 75/8/10
                }
                else
                {
                    document.querySelector(".text2").innerHTML="You can skip "+(i - 1)+" classes. After skipping your attendance will be: " + (this.Classes_attended / (this.Total_classes_held + (i - 1))) * 100+"%"; // Test Case(normal case): 75/10/11
                }
                break;
            }
            ...
            else if (variable_attendance_ratio == this.Criteria)
            {
                ...
                {
                    document.querySelector(".text2").innerHTML="Your attendace is exactly " + this.Criteria * 100 + "% and hence you may skip no classes. If you skip a class your attendance will be: " +(this.Classes_attended / (this.Total_classes_held + 1)) * 100+ "%."; // Test Case: 75/6/8
                }
...
$("button").click(function(){
    ...
    a1 = new attendance(parseFloat(value_3), parseFloat(value_1), parseFloat(value_2));
